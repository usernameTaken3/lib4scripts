import re
mainkeyword=['MICROSCOPIC CROSS SECTIONS', ' T-REG.    1  ===  CROSS SECTION ===']
subkeyword=[' FISSION     ELASTIC SC ', 'AVERAGE']
removespace=[' ELASTIC SC ', ' INELASTIC SC ']
...
for count in range(1, int(filenum)+1):
    ...
    for line in big_file:
        for main in mainkeyword:
            ...
            if mainmatch:
                line=line.replace(mainkeyword[1], '')
        ...
        if print_total: #subkeyword printing
            for sub in subkeyword:
                ...
                if submatch:
                    for space in removespace:
                        wordwithspace=re.findall(space, line)
                        if wordwithspace:
                            #print(wordwithspace[0])
                            wordnospace=str(wordwithspace[0].replace(' ', ''))
                            #print(wordnospace)
                            line=line.replace(wordwithspace[0], wordnospace)
                        else:
                            break
                    small_file.write(line)
                    subcount+=1
